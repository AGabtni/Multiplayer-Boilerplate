...
public class CameraController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //match rotation to orbit angles
    ...
    //Control camera rotation with input
    bool ManualRotation()
    {

        //TODO : bind new keys to input string for IJKL key and AE fr horizontal axis
    ...
    bool AutoRotation()
    {
        ...
        //Get new horizontal orbit angle depending on movement direction
        float headingAngle = GetAngle(movement / Mathf.Sqrt(movementDeltaSqr));

        float deltaAbs = Mathf.Abs(Mathf.DeltaAngle(orbitAngles.y, headingAngle));
        float rotationChange = rotationSpeed * Mathf.Min(Time.unscaledDeltaTime, movementDeltaSqr);

        //Align camera with focus
    ...
    //Clamp vertical orbit angle to settings range and ensure horizontal orbit stays in 0-360 range
    ...
    static float GetAngle(Vector2 direction)
    {
        ...
        //Check if direction is counterclockwise
        return direction.x < 0f ? 360f - angle : angle; ;
    }

    #endregion

    #region Cam collision

    //Return half extends of the camera box cast
